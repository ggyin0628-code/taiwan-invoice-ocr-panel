  ...
  if (mode === "hybrid" || mode === "paddleocr" || mode === "financial-core") {
    ...
    if (paddleOcr.ok) {
      ...
      if (financialOnly) {
        ...
        if (!fullItems.length || fullItems.some((item) => item?.quantity == null)) financialRoiNeeds.push("quantity");
        if (!fullItems.length || fullItems.some((item) => item?.unitPrice == null)) financialRoiNeeds.push("unitPrice");
