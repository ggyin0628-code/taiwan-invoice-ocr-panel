...
function taxIdFromWords(words, text, invoiceNumber) {
...
function tableLineItemsFromWords(words) {
  const allNumeric = numberWords(words)
    .map((word) => ({ ...word, value: Number(word.numeric) }))
    .filter((word) => word.value > 0)
    .sort((a, b) => a.top - b.top || a.left - b.left);
  const numeric = allNumeric
    .filter((word) => word.top >= 200 && word.top <= 390)
    .sort((a, b) => a.top - b.top || a.left - b.left);
  const quantities = numeric.filter((word) => word.value >= 1 && word.value <= 999 && word.left >= 300 && word.left <= 470);
  const unitPrices = numeric.filter((word) => word.value >= 100 && word.value <= 1000000 && word.left >= 390 && word.left <= 570);
  const amounts = numeric.filter((word) => word.value >= 100 && word.value <= 10000000 && word.left >= 580 && word.left <= 780);
  const rows = [];

  for (const unitPrice of unitPrices) {
    const rowCenter = center(unitPrice).y;
    const amount = amounts
      .filter((word) => Math.abs(center(word).y - rowCenter) < 24)
      .sort((a, b) => Math.abs(center(a).y - rowCenter) - Math.abs(center(b).y - rowCenter) || b.value - a.value)[0];
    const quantity = quantities
      .filter((word) => Math.abs(center(word).y - rowCenter) < 42)
      .sort((a, b) => Math.abs(center(a).y - rowCenter) - Math.abs(center(b).y - rowCenter) || a.left - b.left)[0];
    const inferredQuantity = !quantity ? 1 : null;
    const quantityValue = quantity ? quantity.value : inferredQuantity;
    if (!quantityValue) continue;

    const amountValue = amount?.value || unitPrice.value;
    let unitPriceValue = unitPrice.value;
    if (!quantity && amount && amount.value > unitPrice.value * 2) {
      unitPriceValue = amount.value;
    }
    const formulaMatches = Math.abs(quantityValue * unitPriceValue - amountValue) <= Math.max(2, Math.round(amountValue * 0.01));
    if (amount && !formulaMatches) continue;
  ...
  const uniqueRows = [];
  for (const row of rows.sort((a, b) => center(a.words.at(-1)).y - center(b.words.at(-1)).y)) {
    const duplicate = uniqueRows.some((item) => Math.abs(item.amount - row.amount) < 2 && Math.abs(center(item.words.at(-1)).y - center(row.words.at(-1)).y) < 20);
    if (!duplicate) uniqueRows.push(row);
  }

  if (uniqueRows.length) {
    const subtotal = allNumeric
      .filter((word) => word.value >= 1000 && word.left >= 560 && word.left <= 820 && word.top >= 420 && word.top <= 510)
...
function applyTemplateResult(fields, selectedWords, field, result, confidenceFloor = 0.75) {
  ...
  const recognizedConfidence = result.words.length ? confidenceOf(result.words) : 0;
  ...
  selectedWords[field] = result.words;
}

export function extractAnchoredFields({ words = [], text = "", regionOcr = null, templateDetection = null } = {}) {
  ...
  if (regionOcr) {
    applyTemplateResult(fields, selectedWords, "invoiceNumber", invoiceFromWords(regionOcr.invoiceNumber?.words || [], regionOcr.invoiceNumber?.text || ""), 0.85);
  }
  if (!fields.invoiceNumber) applyTemplateResult(fields, selectedWords, "invoiceNumber", invoiceFromWords(words.filter((word) => word.top < 240 && word.left < 560), allText), 0.6);

  if (regionOcr) {
    applyTemplateResult(fields, selectedWords, "taxId", taxIdFromWords(regionOcr.taxId?.words || [], regionOcr.taxId?.rawText || regionOcr.taxId?.text || "", fields.invoiceNumber), 0.85);
  ...
  if (fields.taxId === fields.invoiceNumber.slice(2)) {
    fields.taxId = "";
    fields.confidence.taxId = 0;
    selectedWords.taxId = [];
  }

  if (!fields.taxId) applyTemplateResult(fields, selectedWords, "taxId", taxIdFromWords(words, allText, fields.invoiceNumber), 0.6);

  const tableItems = tableLineItemsFromWords(words);
  ...
  if (tableItems && (!fields.salesAmount || fields.confidence.salesAmount <= tableItems.salesAmount.confidence)) {
