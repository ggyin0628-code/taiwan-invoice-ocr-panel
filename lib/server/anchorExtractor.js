...
export function extractAnchoredFields({ words = [], text = "", regionOcr = null, templateDetection = null, imageWidth = 1200, imageHeight = 700 } = {}) {
  ...
  if (!fields.invoiceNumber) applyTemplateResult(fields, selectedWords, "invoiceNumber", invoiceFromWords(words.filter((word) => word.top < imageHeight * 0.34 && word.left < imageWidth * 0.47), allText), 0.6);

  if (regionOcr) {
    applyTemplateResult(fields, selectedWords, "taxId", taxIdFromWords(regionOcr.taxId?.words || [], regionOcr.taxId?.rawText || regionOcr.taxId?.text || "", fields.invoiceNumber, imageHeight), 0.85);
  ...
  if (!fields.taxId) applyTemplateResult(fields, selectedWords, "taxId", taxIdFromWords(words, allText, fields.invoiceNumber, imageHeight), 0.6);
