...
function normalizeItem(item = {}) {
  ...
  return {
    ...
    confidence: {
      quantity: normalizeConfidence(item.confidence?.quantity ?? item.confidence),
      unitPrice: normalizeConfidence(item.confidence?.unitPrice ?? item.confidence)
    }
  };
}
  ...
  return {
    ...
    identityEvidence: {
      invoiceNumber: {
        ...
        selected: invoiceField.value ?? null,
        resolverReason: invoiceField.resolverReason || ""
      },
      taxId: {
        candidates: Array.isArray(taxIdField.evidence) ? taxIdField.evidence : [],
        selected: taxIdField.value ?? null,
        resolverReason: taxIdField.resolverReason || ""
      }
    }
  };
}
