...
export function mergeInvoiceRecognitionResults({ candidates = [], mode = "local", providerEvents = [] } = {}) {
  ...
  const deterministic = financialScope ? calculateDeterministicFinancials({ items: initialItems }) : null;
  ...
  result.financial = deterministic
    ? { ...financial, calculation: { taxRate: deterministic.taxRate, taxRounding: deterministic.taxRounding, taxPolicyConfirmed: deterministic.taxPolicyConfirmed, secondaryVerification: deterministic.secondaryVerification } }
    : financial;
