...
function chooseItems(candidates, warnings) {
  const validSets = candidates
    .map((candidate) => (candidate.items || []).filter((item) => item.quantity?.status === "auto" || item.unitPrice?.status === "auto" || item.lineAmount?.status === "auto" || item.amount?.status === "auto"))
  ...
  if (conflict) {
    warnings.push("不同 Provider 的金融列資料不一致");
    return first.map((item) => ({
      ...item,
      quantity: item.quantity ? { ...item.quantity, status: "low_confidence", reason: "不同 Provider 結果不一致" } : item.quantity,
      unitPrice: item.unitPrice ? { ...item.unitPrice, status: "low_confidence", reason: "不同 Provider 結果不一致" } : item.unitPrice,
      lineAmount: item.lineAmount ? { ...item.lineAmount, status: "low_confidence", reason: "不同 Provider 結果不一致" } : item.lineAmount,
      amount: item.amount ? { ...item.amount, status: "low_confidence", reason: "不同 Provider 結果不一致" } : item.amount
    }));
  }
  return first;
}
...
export function mergeInvoiceRecognitionResults({ candidates = [], mode = "local", providerEvents = [] } = {}) {
  ...
  const formula = calculateFinancialFields(items, baseCandidates, financialScope);
  const result = {
