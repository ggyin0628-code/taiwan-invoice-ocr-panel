import { buildFinancialCore } from "./financialCore.js";
...
export class InvoiceDocument {
  ...
  toCanonical() {
    ...
    const lineAmounts = items.map((item) => item.lineAmount ?? item.amount).filter((value) => value != null);
    const salesAmount = sales.value != null ? sales.value : lineAmounts.length ? lineAmounts.reduce((sum, value) => sum + value, 0) : null;
    const financial = buildFinancialCore({
      ...
      items,
      salesAmount: { value: salesAmount, status: salesAmount != null ? "auto" : "manual_required", confidence: sales.confidence },
      taxAmount: { value: tax.value, status: tax.value != null ? "auto" : "manual_required", confidence: tax.confidence },
      totalAmount: { value: total.value, status: total.value != null ? "auto" : "manual_required", confidence: total.confidence },
      optionalEvidenceAvailable: items.every((item) => item.quantity != null && item.unitPrice != null && item.itemName),
