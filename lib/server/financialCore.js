...
export function calculateDeterministicFinancials({ items = [], taxRate = COMPANY_TAX_RATE, taxRounding = process.env.FINANCIAL_TAX_ROUNDING || "", taxPolicyConfirmed = process.env.FINANCIAL_TAX_ROUNDING_CONFIRMED === "true", printedSummary = {} } = {}) {
  ...
  return { items: rows, salesAmount, taxAmount, totalAmount, salesValue, taxValue, totalValue: totalAmount.value, complete, taxRate: Number(taxRate), taxRounding: taxRounding || null, taxPolicyConfirmed: Boolean(taxPolicyConfirmed), secondaryVerification: rows.map((row) => row.financialVerification), secondarySummaryVerification };
}
