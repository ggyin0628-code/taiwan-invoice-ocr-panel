...
export function deriveDeterministicFinancialStatus({ sellerTaxId = null, items = [], salesAmount = null, taxAmount = null, totalAmount = null, calculation = null, confidence = 0 } = {}) {
  if (!sellerTaxId?.value || ["invalid", "manual_required", "REVIEW_REQUIRED"].includes(sellerTaxId.status)) return FINANCIAL_STATUS.REVIEW_REQUIRED;
  const rows = Array.isArray(items) ? items : [];
  if (!rows.length || rows.some((item) => {
    const quantity = numericValue(item?.quantity);
    const unitPrice = numericValue(item?.unitPrice);
    const calculated = numericValue(item?.calculatedLineAmount ?? item?.lineAmount ?? item?.amount);
    return !Number.isInteger(quantity) || quantity <= 0 || !Number.isInteger(unitPrice) || unitPrice <= 0 || calculated == null;
  })) return FINANCIAL_STATUS.REVIEW_REQUIRED;
  const verification = [
    ...(calculation?.secondaryVerification || []),
    ...(calculation?.secondarySummaryVerification || []),
    ...rows.map((item) => item.financialVerification).filter(Boolean)
  ];
  if (verification.some((entry) => entry.status === "MISMATCH_REVIEW_REQUIRED")) return FINANCIAL_STATUS.REVIEW_REQUIRED;
  const qOrUReview = rows.some((item) => [item.quantity?.status, item.unitPrice?.status].includes("low_confidence"));
  const sellerReview = sellerTaxId.status === "low_confidence";
  const calculationIncomplete = calculation && calculation.complete === false;
  if (calculationIncomplete) return FINANCIAL_STATUS.REVIEW_REQUIRED;
  const taxNeedsReview = taxAmount?.value == null || totalAmount?.value == null || calculation?.taxPolicyConfirmed !== true;
  const outputReview = [salesAmount, taxAmount, totalAmount].some((fieldValue) => fieldValue?.status === "low_confidence");
  if (qOrUReview || sellerReview || taxNeedsReview || outputReview || confidence < 0.80) return FINANCIAL_STATUS.REVIEW_RECOMMENDED;
...
export function calculateDeterministicFinancials({ items = [], taxRate = COMPANY_TAX_RATE, taxRounding = process.env.FINANCIAL_TAX_ROUNDING || "", taxPolicyConfirmed = process.env.FINANCIAL_TAX_ROUNDING_CONFIRMED === "true", printedSummary = {} } = {}) {
  ...
  const secondarySummaryVerification = [
    ["salesAmount", printedSummary.salesAmount, salesAmount],
    ["taxAmount", printedSummary.taxAmount, taxAmount],
    ["totalAmount", printedSummary.totalAmount, totalAmount]
  ].filter(([, printed, calculated]) => numericValue(printed?.value ?? printed) != null && calculated?.value != null)
    .map(([fieldName, printed, calculated]) => ({
      fieldName,
      printedValue: numericValue(printed?.value ?? printed),
      calculatedValue: calculated.value,
      status: numericValue(printed?.value ?? printed) === calculated.value ? "CALCULATED_VERIFIED" : "MISMATCH_REVIEW_REQUIRED"
    }));
  return { items: rows, salesAmount, taxAmount, totalAmount, salesValue, taxValue, totalValue: totalAmount.value, calculatedLineAmounts: rows.map((row) => row.calculatedLineAmount?.value ?? null), complete, taxRate: Number(taxRate), taxRounding: taxRounding || null, taxPolicyConfirmed: Boolean(taxPolicyConfirmed), secondaryVerification: rows.map((row) => row.financialVerification), secondarySummaryVerification };
}

export function buildFinancialCore({ sellerTaxId = null, buyerTaxId = null, items = [], salesAmount = null, taxAmount = null, totalAmount = null, optionalEvidenceAvailable = true, confidence = 0, calculation = null } = {}) {
  const lineAmounts = (Array.isArray(items) ? items : []).map((item) => item?.amount?.value ?? item?.lineAmount ?? item?.calculatedLineAmount?.value ?? item?.calculatedLineAmount ?? item?.amount).filter((value) => value != null).map((value) => Number(value));
  ...
  const status = calculation
    ? deriveDeterministicFinancialStatus({ sellerTaxId, items, salesAmount, taxAmount, totalAmount, calculation, confidence })
    : deriveFinancialStatus({ sellerTaxId, items, salesAmount, taxAmount, totalAmount, reconciliation, optionalEvidenceAvailable, confidence });
  return {
    sellerTaxId,
    buyerTaxId,
    lineAmounts,
