...
export function extractBuyerTaxIdByPosition(ocrResult) {
  ...
  const picked = label
    ...
    : candidates[0];
  return field(picked.value, { confidence: Math.min(confidence(picked.line.confidence), 0.84), status: "low_confidence", reason: "buyer_tax_id_needs_review", rawText: picked.line.text });
}
...
export function extractInvoiceFieldsFromPaddleOcr(ocrResult) {
  ...
  if (!buyerTaxId.value && /^\d{8}$/.test(digits(ocrResult?.fields?.buyerTaxId?.value))) {
    buyerTaxId = field(digits(ocrResult.fields.buyerTaxId.value), {
      ...
      reason: "buyer_tax_id_needs_review",
...
export async function recognizeInvoiceWithHybrid(imagePath) {
  ...
  const fieldStatuses = {
    invoiceNumber: record.invoiceNumber ? (warnings.includes("invoice_number_missing_prefix") ? "manual_required" : "auto") : "manual_required",
    taxId: record.taxId ? (warnings.includes("buyer_tax_id_same_as_invoice_number") ? "manual_required" : "low_confidence") : "manual_required",
    quantity: record.items?.every((item) => item.quantity != null) && record.items.length ? "auto" : "manual_required",
    unitPrice: record.items?.every((item) => item.unitPrice != null) && record.items.length ? "auto" : "manual_required",
    salesAmount: record.salesAmount != null ? "auto" : "manual_required",
    taxAmount: record.taxAmount != null ? "auto" : "manual_required",
    totalAmount: record.totalAmount != null ? "auto" : "manual_required"
  };
  ...
  const validationErrors = Object.entries(fieldStatuses)
    ...
    .map(([fieldName, status]) => ({ field: fieldName, reason: status === "low_confidence" ? "低信心，需人工確認" : "需人工確認", status, source: record.fieldSources?.[fieldName] || "paddleocr" }));
  return {
    flatFields: {
